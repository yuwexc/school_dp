...
interface Props {
    id: number,
    wordLines: WordLine[],
    setWordLines: Dispatch<SetStateAction<WordLine[]>>,
...
const LineWord: FC<Props> = ({ id, wordLines, setWordLines, register, errors, unregister }) => {
    ...
    const handleDelete = (id: number) => {
        ...
        setWordLines((prevElements) =>
            prevElements.filter((element) => element.id !== id)
        );

        unregister(`russian-${id}`);
        unregister(`transcription-${id}`);
        unregister(`english-${id}`);
    }
    ...
    return (
        <Line>
            <div>
                ...
                {
                    errors[`english-${id}`] && <Message>{errors[`english-${id}`]?.message}</Message>
                }
            </div>
            <div>
                <Input {...register(`transcription-${id}`, {
                    required: {
                        value: true, message: t('sign_up.empty')
                    }
                })} type="text" placeholder="Транскрипция без [ ]" id={'transcription' + id}></Input>
                {
                    errors[`transcription-${id}`] && <Message>{errors[`transcription-${id}`]?.message}</Message>
                }
            </div>
            <div>
                ...
                {
                    errors[`russian-${id}`] && <Message>{errors[`russian-${id}`]?.message}</Message>
                }
            </div>
            <ActionBlock>
